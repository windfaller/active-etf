...
## Public Skill guide and search launch — 2026-09-23

Release 654c410 deployed to the independent Function (zip deployment 0aa5682b-7ad0-43cd-876c-64ef0acf09ce) and SWA (run 35843819859, success). The Function config now reports `releaseStage=production`; no `MCP_BETA_MEMBER_IDS` setting is present. The production package contains only four direct runtime dependencies, with zero production npm audit findings.

Fresh public-origin checks passed: all ten five-language overview/Skill HTML pages return 200 with canonical and hreflang metadata and no noindex header; five credential-free `SKILL.md` files return markdown with noindex; five connection pages retain noindex; robots and sitemap return 200, with ten same-host sitemap URLs. MCP health returns 200, unauthenticated MCP initialize returns 401 with OAuth resource metadata, OAuth discovery has the exact custom-domain issuer. The main site version matches 654c410, its homepage visibly links to MCP and Skill, its old overview/connect routes redirect to the gateway, and it keeps Skill/raw routes hidden. Main-site custom-domain production smoke passed.

Google Search Console's verified `inthewins.com` Domain property accepted `https://active-etf-mcp.inthewins.com/sitemap.xml`, processed ten URLs, discovered the Traditional Chinese Skill page, and accepted a request to index it. Discovery and submission do not mean indexing is complete. Production browser verified the public Skill guide contrast and the signed-out one-prompt install card. Full unit suite: 292 passed, four opt-in Mongo tests skipped. The existing `test:seo` script still fails on its older privacy-page text expectation (`選用成效分析`), unrelated to these MCP pages. A fresh real-member ChatGPT/Grok authorization and seven-tool run was not performed in this release; earlier fixture-backed live tests remain the latest authenticated evidence.
