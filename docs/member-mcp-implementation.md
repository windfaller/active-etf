...
POST /api/agent/install-link requires member session, canonical Origin and CSRF, and is limited to 20 requests/hour/member. It generates a 30-minute opaque instruction URL at /api/agent/install-guide?ticket=... . The guide can be retrieved without browser cookies, contains only installation instructions and the localized Skill payload, and grants no MCP or account access. Tickets are hashed in MongoDB, responses are no-store/noindex, and expired links return 404. Copy a new prompt when it expires. Public five-language Skill guides live at `https://active-etf-mcp.inthewins.com/{locale}/mcp/skill` with downloadable, credential-free `SKILL.md` at `/skills/{locale}/active-etf-research/SKILL.md`; the public file does not authenticate the AI client. The one-prompt ticket remains private to each signed-in member.

The gateway serves its own `robots.txt` and `sitemap.xml` for the five overview and five Skill pages. Connection pages and raw Skill files carry `X-Robots-Tag: noindex`; the main Static Web App redirects its old MCP pages to the gateway and links to them from the homepage and member menu. Set `MCP_RELEASE_STAGE=production` only after validation and with `MCP_BETA_MEMBER_IDS` absent.
...
The portal now separates non-retryable authorization failures (expired/used link or invalid client/request), invalid browser login state, and transient failures. Expired authorization directs the member back to the AI client for a fresh native OAuth flow; invalid login state offers sign-in in the same browser; transient failures retain Retry. Requests time out after 20 seconds rather than waiting indefinitely. Consent is disabled while an error is visible. Only allowlisted diagnostic codes are displayed; arbitrary exception text and callback credentials are not shown.

Bootstrap and downloaded Skill instructions require client-native OAuth discovery, exact resource, fresh state and S256 PKCE; prohibit fabricated/edited/reused authorization links; and tell the client to restart its own OAuth flow after an invalid/expired link. These are agent instructions, not control over a third-party client's implementation. The reported Grok first-attempt cause remains unconfirmed without its error or repair details. No PKCE, redirect, resource, Origin or CSRF validation was relaxed.
