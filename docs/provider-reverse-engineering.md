...
| 安聯投信 `allianz` | `00984A`, `00993A`, `00402A` | Verified and enabled | Official Allianz Angular app calls `AntiForgery/GetAntiForgeryToken` then `Fund/GetFundTradeInfo`; holdings, NAV, AUM, total units, creation unit delta and stock allocation rows were verified. `00402A` maps to `FundNo=E0003`. Production daily refresh is enabled. |
...
| 中信投信 `ctbc` | `00995A`, `00983A`, `00406A` | Verified and enabled | Official CTBC Vue app calls `home/AuthToken` then `etf/Buyback`; holdings, NAV, AUM, total units, creation unit delta and allocation rows were verified. `00406A` maps to `FID=E0038`. Production daily refresh is enabled. |
...
| 復華投信 `fh` | `00991A`, `00409A` | Verified and enabled | Official Fuh Hwa JSON endpoints `/api/assets` and `/api/ETFPcf` were captured from `etf_detail.js` / `util_footer.js`; complete domestic and global holdings, NAV, AUM, total units, allocation rows and PCF unit delta were verified. `00409A` maps to `fundID=ETF26`, including exchange-qualified foreign tickers and ETF components. Production daily refresh is enabled. |
