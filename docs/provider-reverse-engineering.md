...
| 富邦投信 `fubon` | `00982D`, `00983D` | Verified and enabled | Official Fubon ETF assets page renders fixed-income holdings and summary server-side. Bond ISINs and ETF tickers are normalized into the shared holding schema. Production daily refresh is enabled. |
| 聯博投信 `allianceBernstein` | `00984D` | Verified and enabled | Official AllianceBernstein React page calls `webapi.alliancebernstein.com` JSON endpoints `/holdings` and `/basket`; holdings, NAV, AUM, total units and PCF summary were verified. Production daily refresh is enabled. |
