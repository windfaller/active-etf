...
const selectedCodes = ref<string[]>([...props.codes]);
...
function changeType(type: "tw" | "global"): void { selectedType.value = type; selectedCodes.value = []; comparison.value = null; }
...
watch(() => [props.type, props.codes.join(","), props.refreshKey], () => {
  selectedType.value = props.type; selectedCodes.value = [...props.codes];
  if (props.codes.length >= 2 && props.codes.length <= 4) void load(props.type, props.codes);
}, { immediate: true });
