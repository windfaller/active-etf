  ...
  <section class="compare-view">
    ...
    <section v-if="comparison" class="etf-card-grid">
      <article v-for="card in comparison.cards" :key="card.code" class="etf-card">
        ...
        <div v-else-if="metric==='holdings'" class="rank-list"><template v-for="(holding,index) in card.topHoldings.slice(0,8)" :key="holding.key"><MemberLockedResult v-if="shouldMaskMemberResult(isAuthenticated,index)" compact title="比較持股已遮隱" :source="`compare_holding_${index + 1}`" /><span v-else><b>{{ holding.symbol }} {{ holding.name }}</b><small>{{ holding.assetType }}｜{{ formatWeight(holding.weight) }}</small></span></template></div>
        <div v-else-if="metric==='sectors'" class="rank-list"><template v-for="(sector,index) in card.sectorExposure.slice(0,8)" :key="sector.sector"><MemberLockedResult v-if="shouldMaskMemberResult(isAuthenticated,index)" compact title="比較產業已遮隱" :source="`compare_sector_${index + 1}`" /><span v-else><b>{{ sector.sector }}</b><small>{{ formatWeight(sector.weight) }}</small></span></template><template v-if="card.assetComposition"><template v-for="(asset,index) in card.assetComposition" :key="asset.assetType"><MemberLockedResult v-if="shouldMaskMemberResult(isAuthenticated,index)" compact title="資產組成已遮隱" :source="`compare_asset_${index + 1}`" /><span v-else><b>{{ asset.assetType }}</b><small>{{ formatWeight(asset.weight) }}</small></span></template></template></div>
        <div v-else class="rank-list"><template v-if="card.activeAdjustments"><template v-for="(item,index) in card.activeAdjustments" :key="item.window"><MemberLockedResult v-if="shouldMaskMemberResult(isAuthenticated,index)" compact title="調整資料已遮隱" :source="`compare_activity_${index + 1}`" /><span v-else><b :class="valueTone(item.cumulativeActiveNetLots)">{{ item.window }} 日主動 {{ formatNumber(item.cumulativeActiveNetLots) }} 張</b><small>調整強度 {{ formatNumber(item.adjustmentIntensity,2) }}｜加碼持股筆數 {{ item.increaseHoldingChangeCount }}｜減碼持股筆數 {{ item.decreaseHoldingChangeCount }}</small></span></template><MemberLockedResult v-if="!isAuthenticated" compact title="當期調整廣度已遮隱" source="compare_breadth" /><span v-else><b>新增 {{ card.addedHoldings }}／退出 {{ card.exitedHoldings }}</b><small>當期調整廣度</small></span></template><template v-else><MemberLockedResult v-if="!isAuthenticated" compact title="權重調整強度已遮隱" source="compare_intensity" /><span v-else><b>權重調整強度 {{ formatNumber(card.weightAdjustmentIntensity,2) }}</b><small>Σ |weight change| / 2</small></span></template></div>
      </article>
    </section>

    <section v-if="comparison" class="pairwise-panel"><div class="section-title"><Layers3 :size="20" /><div><h2>Pairwise 重疊矩陣</h2><p>手機使用比較卡，不需要橫向捲動四檔寬表格。</p></div></div><div class="pair-grid"><template v-for="(pair,index) in comparison.pairwise" :key="`${pair.left}-${pair.right}`"><MemberLockedResult v-if="shouldMaskMemberResult(isAuthenticated,index)" title="重疊比較已遮隱" :source="`compare_pair_${index + 1}`" /><article v-else><header><b>{{ pair.left }} × {{ pair.right }}</b><span>共同 {{ pair.intersectionCount }} / 聯集 {{ pair.unionCount }}</span></header><div><span>Jaccard</span><strong>{{ pair.similarity === null ? '-' : formatNumber(pair.similarity * 100,1) + '%' }}</strong></div><div><span>權重重疊</span><strong>{{ formatWeight(pair.weightedOverlap) }}</strong></div><details><summary>共同持股 {{ pair.common.length }} 檔</summary><p v-for="holding in pair.common" :key="holding.key"><b>{{ holding.label }}</b><span>{{ pair.left }} {{ formatWeight(holding.leftWeight) }}｜{{ pair.right }} {{ formatWeight(holding.rightWeight) }}</span></p></details></article></template></div></section>
