...
import { formatSignedPp, formatWeight } from "../utils/format";
...
<template>
  <section class="global-market-view view-shell">
    ...
    <button class="institution-entry" type="button" @click="emit('institutions')"><Building2 :size="24" /><span><b>前往機構 13F</b><small>季度持倉、持倉截止日與延遲天數</small></span><ArrowRight :size="19" /></button>
    ...
    <section class="data-panel"><div class="panel-heading"><div><h2>海外 ETF 共同持有</h2><p>{{ selectedDate || report?.reportDate || "-" }}｜只比較 ETF，不混入 13F。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 共同持有" :empty="!loading && !commonRows.length">
        <div class="global-table"><div class="table-head"><span>標的</span><span>持有 ETF</span><span>合計權重</span><span>最高權重</span></div><div v-for="row in commonRows" :key="row.ticker ?? row.name" class="table-row"><span class="stock"><b>{{ row.ticker ?? "-" }}</b><small>{{ row.name }}</small></span><span class="chips"><button v-for="etf in row.etfs" :key="etf.code" type="button" @click="emit('etf',etf.code)">{{ etf.code }}</button></span><span>{{ formatWeight(row.total,1) }}</span><span>{{ formatWeight(row.max,1) }}</span></div></div>
        <template #mobile><MobileDataCard v-for="row in commonRows" :key="row.ticker ?? row.name" :label="row.ticker ?? row.name"><template #title>{{ row.ticker ?? "-" }} {{ row.name }}</template><template #summary>{{ row.etfs.length }} 檔 ETF 共同持有｜合計 {{ formatWeight(row.total,1) }}</template><p>最高單檔權重 {{ formatWeight(row.max,1) }}</p><p><button v-for="etf in row.etfs" :key="etf.code" type="button" class="chip" @click="emit('etf',etf.code)">{{ etf.code }} {{ formatWeight(etf.weight,1) }}</button></p></MobileDataCard></template>
      </ResponsiveDataTable>
    </section>
    <section class="data-panel"><div class="panel-heading"><div><h2>海外 ETF 共同權重變化</h2><p>使用 ETF 持股權重變化，不將被動 ETF 寫成經理人主動加碼。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 共同權重變化" :empty="!loading && !moverRows.length">
        <div class="global-table"><div class="table-head"><span>標的</span><span>影響 ETF</span><span>方向</span><span>合計變化</span></div><div v-for="row in moverRows" :key="row.ticker ?? row.name" class="table-row"><span class="stock"><b>{{ row.ticker ?? "-" }}</b><small>{{ row.name }}</small></span><span class="chips"><button v-for="code in row.etfs" :key="code" type="button" @click="emit('etf',code)">{{ code }}</button></span><span>{{ row.delta > 0 ? '增加 ▲' : row.delta < 0 ? '減少 ▼' : '持平' }}</span><span :class="row.delta >= 0 ? 'positive' : 'negative'">{{ formatSignedPp(row.delta,1) }}</span></div></div>
        <template #mobile><MobileDataCard v-for="row in moverRows" :key="row.ticker ?? row.name" :label="row.ticker ?? row.name" :tone="row.delta > 0 ? 'increase' : row.delta < 0 ? 'decrease' : 'neutral'"><template #title>{{ row.ticker ?? "-" }} {{ row.name }}</template><template #summary>{{ row.delta > 0 ? '權重增加 ▲' : '權重減少 ▼' }} {{ formatSignedPp(row.delta,1) }}｜{{ row.etfs.length }} 檔 ETF</template><p><button v-for="code in row.etfs" :key="code" type="button" class="chip" @click="emit('etf',code)">{{ code }}</button></p></MobileDataCard></template>
      </ResponsiveDataTable>
    </section>
