  ...
  <section class="global-market-view view-shell">
    ...
    <section class="data-panel"><div class="panel-heading"><div><h2>海外 ETF 共同持有</h2><p>{{ selectedDate || report?.reportDate || "-" }}｜只比較 ETF，不混入 13F。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 共同持有" :empty="!loading && !commonRows.length">
        <div class="global-table"><div class="table-head"><span>標的</span><span>持有 ETF</span><span>合計權重</span><span>最高權重</span></div><div v-for="row in commonRows" :key="row.ticker ?? row.name" class="table-row"><span class="stock"><b>{{ row.ticker ?? "-" }}</b><small>{{ row.name }}</small></span><span class="chips"><a v-for="etf in row.etfs" :key="etf.code" :href="`/global-etfs/${etf.code}`" @click.prevent="emit('etf',etf.code)">{{ etf.code }}</a></span><span>{{ formatWeight(row.total,1) }}</span><span>{{ formatWeight(row.max,1) }}</span></div></div>
    ...
    <section class="data-panel"><div class="panel-heading"><div><h2>海外 ETF 共同權重變化</h2><p>使用 ETF 持股權重變化，不將被動 ETF 寫成經理人主動加碼。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 共同權重變化" :empty="!loading && !moverRows.length">
        <div class="global-table"><div class="table-head"><span>標的</span><span>影響 ETF</span><span>方向</span><span>合計變化</span></div><div v-for="row in moverRows" :key="row.ticker ?? row.name" class="table-row"><span class="stock"><b>{{ row.ticker ?? "-" }}</b><small>{{ row.name }}</small></span><span class="chips"><a v-for="code in row.etfs" :key="code" :href="`/global-etfs/${code}`" @click.prevent="emit('etf',code)">{{ code }}</a></span><span :class="valueTone(row.delta)">{{ row.delta > 0 ? '增加 ▲' : row.delta < 0 ? '減少 ▼' : '持平' }}</span><span :class="valueTone(row.delta)">{{ formatSignedPp(row.delta,1) }}</span></div></div>
