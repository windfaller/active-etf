          ...
          <MobileDataCard v-else :id="`market-stock-${row.stockId}`" class="market-impact-card" :class="{ 'is-focused': focusStockId === row.stockId }" :label="`${row.stockId} ${row.stockName}`" :tone="row.totalActiveDiffLots > 0 ? 'increase' : row.totalActiveDiffLots < 0 ? 'decrease' : 'neutral'" :expandable="false">
            ...
            <template #summary>
              <span class="impact-summary-primary">
                <b :class="valueTone(row.totalActiveDiffLots)">{{ directionLabel(row.totalActiveDiffLots) }} {{ formatLots(row.totalActiveDiffLots) }} 張</b>
                <span :class="valueTone(row.totalDiffWeightPoint)">權重 {{ formatSignedPp(row.totalDiffWeightPoint) }}</span>
              </span>
            ...
            <dl class="impact-card-facts">
              ...
              <div class="impact-card-fact impact-card-source">
                ...
                <dd class="impact-source-list">
                  <button v-for="etf in row.etfs.slice(0,5)" :key="etf.etfCode" type="button" class="chip source-chip" @click="emit('etf', etf.etfCode)">{{ etf.etfCode }} <span :class="valueTone(etf.activeDiffLots ?? etf.diffLots)">{{ formatLots(etf.activeDiffLots ?? etf.diffLots) }} 張</span></button>
