    ...
    <template v-if="page === 'premiumHistory'">
      <section class="data-panel">
        ...
        <ResponsiveDataTable label="ETF 折溢價歷史" :empty="!loading && !premiumRows.length">
          ...
          <template #mobile><MobileDataCard v-for="row in premiumRows" :key="row.tradeDate" :label="row.tradeDate" :tone="(row.premiumDiscount ?? 0) >= 0 ? 'increase' : 'decrease'" :expandable="false"><template #title>{{ row.tradeDate }}</template><template #summary>{{ (row.premiumDiscount ?? 0) >= 0 ? '溢價' : '折價' }} {{ formatSignedPp(row.premiumDiscount) }}<br />市價 {{ formatNumber(row.marketPrice,2) }}｜淨值 {{ formatNumber(row.nav,2) }}</template></MobileDataCard></template>
        </ResponsiveDataTable>
      </section>
    </template>
