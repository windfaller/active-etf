...
import { directionLabel, formatLots, formatMoney, formatNumber, formatSignedPp, formatWeight } from "../utils/format";
  ...
  <section class="view-shell taiwan-etf-view">
    ...
    <template v-if="page === 'premiumHistory'">
      <section class="data-panel">
        ...
        <ResponsiveDataTable label="ETF 折溢價歷史" :empty="!loading && !premiumRows.length">
          <div class="simple-table"><div class="table-head"><span>交易日</span><span>市價</span><span>淨值</span><span>折溢價</span></div><div v-for="row in premiumRows" :key="row.tradeDate" class="table-row"><span>{{ row.tradeDate }}</span><span>{{ formatNumber(row.marketPrice,2) }}</span><span>{{ formatNumber(row.nav,2) }}</span><span :class="(row.premiumDiscount ?? 0) >= 0 ? 'positive' : 'negative'">{{ formatSignedPp(row.premiumDiscount) }}</span></div></div>
          <template #mobile><MobileDataCard v-for="row in premiumRows" :key="row.tradeDate" :label="row.tradeDate" :tone="(row.premiumDiscount ?? 0) >= 0 ? 'increase' : 'decrease'"><template #title>{{ row.tradeDate }}</template><template #summary>{{ (row.premiumDiscount ?? 0) >= 0 ? '溢價' : '折價' }} {{ formatSignedPp(row.premiumDiscount) }}</template><p>市價 {{ formatNumber(row.marketPrice,2) }}｜淨值 {{ formatNumber(row.nav,2) }}</p></MobileDataCard></template>
        </ResponsiveDataTable>
      </section>
    </template>
    ...
    <template v-else>
      <section class="summary-grid">
        ...
        <article><span>折溢價</span><strong>{{ formatSignedPp(summary?.premiumDiscount) }}</strong><small>市價 {{ formatNumber(summary?.marketPrice,2) }}｜淨值 {{ formatNumber(summary?.nav,2) }}</small></article>
      ...
      <section id="changes-panel" class="data-panel">
        ...
        <ResponsiveDataTable label="單檔 ETF 調倉" :empty="!loading && !operationRows.length">
          <div class="change-table"><div class="table-head"><span>股票</span><span>操作</span><span>主動淨變動</span><span>表面變動</span><span>權重變動</span><span>目前權重</span></div><div v-for="row in operationRows" :key="row.stockId" class="table-row"><span class="stock"><b>{{ row.stockId }}</b><small>{{ row.stockName }}</small></span><span>{{ operationStatus(row) }}</span><span :class="(row.activeDiffLots ?? row.diffLots) >= 0 ? 'positive' : 'negative'">{{ formatLots(row.activeDiffLots ?? row.diffLots) }} 張</span><span>{{ formatLots(row.diffLots) }} 張</span><span>{{ formatSignedPp(row.diffWeightPoint) }}</span><span>{{ formatWeight(row.currentWeight) }}</span></div></div>
          <template #mobile><MobileDataCard v-for="row in operationRows" :key="row.stockId" :label="`${row.stockId} ${row.stockName}`" :tone="(row.activeDiffLots ?? row.diffLots) > 0 ? 'increase' : 'decrease'"><template #title>{{ row.stockId }} {{ row.stockName }}</template><template #summary>{{ operationStatus(row) }}｜主動 {{ formatLots(row.activeDiffLots ?? row.diffLots) }} 張｜權重 {{ formatSignedPp(row.diffWeightPoint) }}</template><p>表面持股變動 {{ formatLots(row.diffLots) }} 張</p><p>目前權重 {{ formatWeight(row.currentWeight) }}</p></MobileDataCard></template>
        </ResponsiveDataTable>
      </section>

      <section v-if="section === 'overview'" class="data-panel">
        <div class="panel-heading"><div><h2><Database :size="19" /> 單檔 ETF 持股</h2><p>依目前權重排序，手機版可展開查看股數與市值。</p></div><label class="holding-search"><Search :size="15" /><input v-model="holdingQuery" type="search" placeholder="搜尋持股" /></label></div>
        <ResponsiveDataTable label="單檔 ETF 持股" :empty="!loading && !displayedHoldings.length">
          ...
          <template #mobile><MobileDataCard v-for="row in displayedHoldings" :key="row.stockId" :label="`${row.stockId} ${row.stockName}`"><template #title>{{ row.stockId }} {{ row.stockName }}</template><template #summary>權重 {{ formatWeight(row.weight) }}｜{{ formatNumber(row.lots) }} 張｜市值 {{ formatMoney(row.marketValue) }}</template><p>持股股數 {{ formatNumber(row.shares) }}</p></MobileDataCard></template>
        </ResponsiveDataTable>
      </section>
    </template>
