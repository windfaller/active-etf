    ...
    <template v-else>
      ...
      <section id="changes-panel" class="data-panel">
        ...
        <ResponsiveDataTable label="單檔 ETF 調倉" :empty="!loading && !operationRows.length">
          ...
          <template #mobile><MobileDataCard v-for="row in operationRows" :key="row.stockId" :label="`${row.stockId} ${row.stockName}`" :tone="(row.activeDiffLots ?? row.diffLots) > 0 ? 'increase' : 'decrease'" :expandable="false"><template #title>{{ row.stockId }} {{ row.stockName }}</template><template #summary>{{ operationStatus(row) }}｜主動 {{ formatLots(row.activeDiffLots ?? row.diffLots) }} 張<br />權重變化 {{ formatSignedPp(row.diffWeightPoint) }}｜目前 {{ formatWeight(row.currentWeight) }}</template></MobileDataCard></template>
        </ResponsiveDataTable>
      </section>
