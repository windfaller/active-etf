...
<template>
  <section class="institution-view view-shell">
    ...
    <template v-else-if="section">
      ...
      <section class="data-panel"><div class="panel-heading"><div><h2>季度持倉變化</h2><p>13F 專用語：季度持倉變化。</p></div></div><ResponsiveDataTable label="13F 季度持倉變化" :empty="!loading && !section.weightChanges.length"><div class="change-list"><div v-for="row in section.weightChanges" :key="row.positionKey ?? row.ticker ?? row.name"><span><b>{{ row.ticker ?? "-" }}</b>{{ row.name }}</span><span>{{ formatWeight(row.currentWeightPercent,1) }}</span><strong :class="valueTone(row.deltaPp)">{{ (row.deltaPp ?? 0) > 0 ? '增持 ▲' : (row.deltaPp ?? 0) < 0 ? '減持 ▼' : '持平' }} {{ formatSignedPp(row.deltaPp,1) }}</strong></div></div><template #mobile><MobileDataCard v-for="row in section.weightChanges" :key="row.positionKey ?? row.ticker ?? row.name" :label="row.ticker ?? row.name" :tone="(row.deltaPp ?? 0)>0?'increase':(row.deltaPp ?? 0)<0?'decrease':'neutral'" :expandable="false"><template #title>{{ row.ticker ?? "-" }} {{ row.name }}</template><template #summary><span :class="valueTone(row.deltaPp)">季度{{ (row.deltaPp ?? 0)>0?'增持 ▲':(row.deltaPp ?? 0)<0?'減持 ▼':'持平' }} {{ formatSignedPp(row.deltaPp,1) }}</span>｜目前 {{ formatWeight(row.currentWeightPercent,1) }}</template></MobileDataCard></template></ResponsiveDataTable></section>
