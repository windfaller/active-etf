  ...
  <section class="global-etf-view view-shell">
    ...
    <section class="data-panel"><div class="panel-heading"><div><h2><Database :size="19" /> 官方持股</h2><p>權重變化是與前期官方持股比較，不一律代表經理人主動決策。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 持股" :empty="!loading && !section?.topHoldings.length">
        ...
        <template #mobile><MobileDataCard v-for="(row,index) in section?.topHoldings ?? []" :key="row.ticker ?? row.name" :label="row.ticker ?? row.name" :tone="(deltaFor(row.ticker,row.name) ?? 0) > 0 ? 'increase' : (deltaFor(row.ticker,row.name) ?? 0) < 0 ? 'decrease' : 'neutral'"><template #title>{{ row.ticker ?? "-" }} {{ row.name }}</template><template #summary>權重 {{ formatWeight(row.weightPercent,1) }}｜較前期 {{ formatSignedPp(deltaFor(row.ticker,row.name),1) }}<br />{{ componentSummary(index) || row.assetType || row.sector || "-" }}</template><p>市值 {{ formatMoney(row.marketValue) }}</p></MobileDataCard></template>
      </ResponsiveDataTable>
    </section>
    <section class="data-panel"><div class="panel-heading"><div><h2>持股權重變化</h2><p>海外 ETF 使用「持股權重變化」用語。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 權重變化" :empty="!loading && !section?.weightChanges.length"><div class="change-list"><div v-for="row in section?.weightChanges ?? []" :key="row.positionKey ?? row.ticker ?? row.name"><span><b>{{ row.ticker ?? "-" }}</b>{{ row.name }}</span><span>{{ formatWeight(row.currentWeightPercent,1) }}</span><strong :class="(row.deltaPp ?? 0) >= 0 ? 'positive' : 'negative'">{{ (row.deltaPp ?? 0) >= 0 ? '增加 ▲' : '減少 ▼' }} {{ formatSignedPp(row.deltaPp,1) }}</strong></div></div><template #mobile><MobileDataCard v-for="row in section?.weightChanges ?? []" :key="row.positionKey ?? row.ticker ?? row.name" :label="row.ticker ?? row.name" :tone="(row.deltaPp ?? 0) > 0 ? 'increase' : 'decrease'"><template #title>{{ row.ticker ?? "-" }} {{ row.name }}</template><template #summary>{{ (row.deltaPp ?? 0) >= 0 ? '權重增加 ▲' : '權重減少 ▼' }} {{ formatSignedPp(row.deltaPp,1) }}｜目前 {{ formatWeight(row.currentWeightPercent,1) }}</template></MobileDataCard></template></ResponsiveDataTable>
    </section>
