  ...
  <section class="global-etf-view view-shell">
    ...
    <section class="data-panel"><div class="panel-heading"><div><h2><Database :size="19" /> 官方持股</h2><p>權重變化是與前期官方持股比較，不一律代表經理人主動決策。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 持股" :empty="!loading && !section?.topHoldings.length">
        <div class="global-table"><div class="table-head"><span>標的</span><span>名稱</span><span>權重</span><span>較前期</span><span>類型／組成</span></div><div v-for="(row,index) in section?.topHoldings ?? []" :key="row.ticker ?? row.name" class="table-row"><span><b>{{ row.ticker ?? "-" }}</b></span><span>{{ row.name }}</span><span>{{ formatWeight(row.weightPercent,1) }}</span><span :class="valueTone(deltaFor(row.ticker,row.name))">{{ formatSignedPp(deltaFor(row.ticker,row.name),1) }}</span><span>{{ componentSummary(index) || row.sector || row.assetType || "-" }}</span></div></div>
        <template #mobile><MobileDataCard v-for="(row,index) in section?.topHoldings ?? []" :key="row.ticker ?? row.name" :label="row.ticker ?? row.name" :tone="(deltaFor(row.ticker,row.name) ?? 0) > 0 ? 'increase' : (deltaFor(row.ticker,row.name) ?? 0) < 0 ? 'decrease' : 'neutral'" :expandable="false"><template #title>{{ row.ticker ?? "-" }} {{ row.name }}</template><template #summary>權重 {{ formatWeight(row.weightPercent,1) }}｜<span :class="valueTone(deltaFor(row.ticker,row.name))">較前期 {{ formatSignedPp(deltaFor(row.ticker,row.name),1) }}</span><br />{{ componentSummary(index) || row.assetType || row.sector || "-" }}｜市值 {{ formatMoney(row.marketValue) }}</template></MobileDataCard></template>
      </ResponsiveDataTable>
    </section>
    <section class="data-panel"><div class="panel-heading"><div><h2>持股權重變化</h2><p>海外 ETF 使用「持股權重變化」用語。</p></div></div>
      <ResponsiveDataTable label="海外 ETF 權重變化" :empty="!loading && !section?.weightChanges.length"><div class="change-list"><div v-for="row in section?.weightChanges ?? []" :key="row.positionKey ?? row.ticker ?? row.name"><span><b>{{ row.ticker ?? "-" }}</b>{{ row.name }}</span><span>{{ formatWeight(row.currentWeightPercent,1) }}</span><strong :class="valueTone(row.deltaPp)">{{ (row.deltaPp ?? 0) > 0 ? '增加 ▲' : (row.deltaPp ?? 0) < 0 ? '減少 ▼' : '持平' }} {{ formatSignedPp(row.deltaPp,1) }}</strong></div></div><template #mobile><MobileDataCard v-for="row in section?.weightChanges ?? []" :key="row.positionKey ?? row.ticker ?? row.name" :label="row.ticker ?? row.name" :tone="(row.deltaPp ?? 0) > 0 ? 'increase' : (row.deltaPp ?? 0) < 0 ? 'decrease' : 'neutral'" :expandable="false"><template #title>{{ row.ticker ?? "-" }} {{ row.name }}</template><template #summary><span :class="valueTone(row.deltaPp)">{{ (row.deltaPp ?? 0) > 0 ? '權重增加 ▲' : (row.deltaPp ?? 0) < 0 ? '權重減少 ▼' : '權重持平' }} {{ formatSignedPp(row.deltaPp,1) }}</span>｜目前 {{ formatWeight(row.currentWeightPercent,1) }}</template></MobileDataCard></template></ResponsiveDataTable>
    </section>
