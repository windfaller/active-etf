...
<template>
  <section class="stock-view">
    ...
    <section v-if="overview" class="p1-panel"><div class="panel-title"><Database :size="20" /><div><h2>ETF 明細</h2><p>{{ market === 'tw' ? '持股與規模校正後調整' : '相同 ticker 仍依 exposure 類型分開' }}</p></div></div><ResponsiveDataTable label="股票 ETF 明細" :empty="!loading && !(etfs?.rows.length)"><div class="etf-detail-table"><div class="table-head"><span>ETF</span><span>權重</span><span>權重變化</span><span>{{ market === 'tw' ? '主動淨變動' : 'Exposure' }}</span><span>連續方向</span><span>資料日／信心</span></div><div v-for="row in etfs?.rows ?? []" :key="`${row.etfCode}-${row.assetType ?? ''}`" class="table-row"><span><b>{{ row.etfCode }}</b><small>{{ row.name }}</small></span><span>{{ formatWeight(row.latestWeight) }}</span><span>{{ formatSignedPp(row.weightChange) }}</span><span>{{ market === 'tw' ? `${formatLots(row.activeNetLots)} 張` : (row.assetType ?? '-') }}</span><span>{{ row.consecutiveTradingDays ? `${row.consecutiveTradingDays} 日 ${row.consecutiveDirection === 'increase' ? '加碼' : '減碼'}` : '-' }}</span><span>{{ row.dataDate ?? '-' }}<small>信心 {{ confidenceLabel(row.confidence) }}</small></span></div></div><template #mobile><MobileDataCard v-for="row in etfs?.rows ?? []" :key="`${row.etfCode}-${row.assetType ?? ''}`" :label="row.etfCode" :tone="(row.activeNetLots ?? row.weightChange ?? 0) >= 0 ? 'increase' : 'decrease'" :expandable="false"><template #title>{{ row.etfCode }} {{ row.name }}</template><template #summary>權重 {{ formatWeight(row.latestWeight) }}｜變化 {{ formatSignedPp(row.weightChange) }}<br />{{ market === 'tw' ? `主動 ${formatLots(row.activeNetLots)} 張` : `類型 ${row.assetType ?? '-'}` }}｜{{ row.dataDate ?? '-' }}</template></MobileDataCard></template></ResponsiveDataTable></section>
