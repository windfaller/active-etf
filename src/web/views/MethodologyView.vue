...
const sections = [
  ...
  { title: "3／5／20 日連續與反轉", fact: "window 使用有效市場交易日，不使用日曆天。", calculation: "主動淨變動絕對值 ≤ 0.01 張且權重變化絕對值 ≤ 0.0001 pp 時為 neutral。反轉前至少 2 個有效交易日同方向，反轉日需跨過門檻。", limit: "來源延遲或缺失日不會被補成零，也不強制算成正負方向。" },
  ...
  { title: "持股集合與權重重疊", fact: "集合重疊顯示 intersection count 與 Jaccard similarity。", calculation: "Jaccard = 交集 / 聯集；Weighted overlap = Σ min(weightA, weightB)。", limit: "缺權重持股只進入集合重疊。海外 positionKey 會和 assetType 合併識別，Equity、Swap、Cash 不靜默合併。" },
  ...
  { title: "資料可信度", fact: "每個 API 顯示 tracked、available、delayed 與 sourceAsOf。", calculation: "高／中／低依涵蓋率、規模校正完整度、觀察期、延遲、單一 ETF 主導程度與同方向比例判定，並附文字原因。", limit: "本站不使用無方法論的神秘分數。" },
