...
import { formatLots, formatNumber, formatSigned } from "../utils/format";
...
<template>
  <section class="signals-view">
    <header class="signals-hero"><span>交易日訊號</span><h1>連續調倉、反轉與方向分歧</h1><p>訊號只使用有效市場交易日；neutral 不稀釋共識比例，但會獨立列出。研究解讀不代表買賣建議或未來報酬預測。</p></header>
    <nav class="signal-tabs" aria-label="訊號頁籤"><button v-for="tab in tabs" :key="tab.kind" type="button" :class="{active:kind===tab.kind}" @click="emit('navigate',tab.path)">{{ tab.label }}</button><label>觀察期<select v-model.number="windowSize"><option :value="3">3 日</option><option :value="5">5 日</option><option :value="20">20 日</option></select></label></nav>
    ...
    <section v-if="signals && showConsecutive" class="signal-section"><div class="section-title"><Repeat2 :size="21" /><div><h2>連續加碼／減碼</h2><p>至少連續 2 個有效觀察日，最新日仍需跨過 neutral 門檻。</p></div></div><div v-if="signals.consecutive.length" class="signal-grid"><button v-for="row in signals.consecutive" :key="row.stock.symbol" type="button" @click="emit('navigate',row.stock.path)"><header><span :class="row.direction">{{ directionLabel(row.direction) }}</span><b>{{ row.stock.symbol }} {{ row.stock.name }}</b></header><strong>連續 {{ row.consecutiveTradingDays }} 日</strong><p>累積主動 {{ formatLots(row.cumulativeActiveNetLots) }} 張</p><footer><span>{{ row.participatingEtfs }} 檔參與｜同向 {{ row.sameDirectionEtfRatio === null ? '-' : formatNumber(row.sameDirectionEtfRatio*100,0)+'%' }}</span><small>信心 {{ confidenceLabel(row.confidence.level) }}｜{{ row.startDate }} → {{ row.latestDate }}</small></footer></button></div><p v-else class="signal-state">此觀察期沒有達到條件的連續訊號。</p></section>

    <section v-if="signals && showReversals" class="signal-section"><div class="section-title"><ArrowRightLeft :size="21" /><div><h2>反轉訊號</h2><p>反轉前至少 2 個有效交易日同方向，不以單日正負號變化判定。</p></div></div><div v-if="signals.reversals.length" class="signal-grid"><button v-for="row in signals.reversals" :key="row.stock.symbol" type="button" @click="emit('navigate',row.stock.path)"><header><span class="reversal">反轉</span><b>{{ row.stock.symbol }} {{ row.stock.name }}</b></header><strong>{{ row.reversalType }}</strong><p>反轉前 {{ formatLots(row.beforeActiveNetLots) }} 張 → 當日 {{ formatLots(row.afterActiveNetLots) }} 張</p><footer><span>前段連續 {{ row.priorTradingDays }} 日｜{{ row.majorityEtfDirectionFlip ? '多數 ETF 同步翻轉' : '未形成多數翻轉' }}</span><small>{{ row.reversalDate }}｜信心 {{ confidenceLabel(row.confidence.level) }}</small></footer></button></div><p v-else class="signal-state">此觀察期沒有達到條件的反轉訊號。</p></section>

    <section v-if="signals && showDivergence" class="signal-section"><div class="section-title"><Split :size="21" /><div><h2>ETF 與法人分歧</h2><p>ETF 加碼遇法人賣超，或 ETF 減碼遇法人買超；缺任一側資料即為資料不足。</p></div></div><div v-if="signals.divergences.length" class="signal-grid"><button v-for="row in signals.divergences" :key="row.stock.symbol" type="button" @click="emit('navigate',row.stock.path)"><header><span class="divergent">分歧</span><b>{{ row.stock.symbol }} {{ row.stock.name }}</b></header><strong>ETF {{ directionLabel(row.etfDirection) }} × 法人 {{ (row.institutionNetShares ?? 0) > 0 ? '買超' : '賣超' }}</strong><p>法人合計 {{ formatSigned(row.institutionNetShares) }} 股</p><footer><span>{{ row.changedFromAligned ? '由一致轉為分歧' : '當日方向分歧' }}</span><small>{{ row.date }}｜信心 {{ confidenceLabel(row.confidence.level) }}</small></footer></button></div><p v-else class="signal-state">此觀察期沒有可確認的 ETF／法人分歧。</p></section>
