...
const props = defineProps<{locale: AgentLocale; authenticated: boolean; endpoint: string}>();
...
<template>
  <section class="agent-install" aria-labelledby="install-title">
    <h2 id="install-title">{{ text.title }}</h2>
...
.agent-install select { display:block; margin: 10px 0 20px; max-width:100%; padding:12px; font:inherit; border:1px solid #b8cccc; border-radius:8px; background:white; color:inherit; }
.install-command { display:flex; gap:12px; align-items:center; margin:16px 0; }
.install-command pre { flex:1; min-width:0; white-space:pre-wrap; overflow-wrap:anywhere; padding:14px; background:#eff6f5; border-radius:8px; }
.install-command button { flex-shrink:0; }
.agent-install details { margin:20px 0; }
.agent-install summary { cursor:pointer; }
.agent-install fieldset { margin-top:24px; border:1px solid #d9e5e5; border-radius:10px; padding:16px; }
