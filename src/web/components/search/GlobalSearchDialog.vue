...
const AdSlot = defineAsyncComponent(() => import("../../../components/ads/AdSlot"));
...
<template>
  <section class="global-search" :class="mode">
    ...
    <aside v-if="mode === 'dialog'" class="search-sponsored" aria-label="合作內容">
      <small>合作內容</small>
      <AdSlot slot="article-inline" mode="compact" compact />
    </aside>
    ...
    <div v-if="results.length" class="search-results" role="listbox" aria-label="搜尋結果">
      <a v-for="result in results" :key="`${result.type}-${result.code}`" :href="result.path" @click.prevent="choose(result.path)"><span class="result-type">{{ result.typeLabel }}</span><span><b>{{ result.code }} {{ result.name }}</b><small>{{ result.market }}｜資料日 {{ result.latestDataDate ?? '依目的頁標示' }}</small></span><ArrowRight :size="17" /></a>
    </div>
...
.search-results a{display:grid;grid-template-columns:90px 1fr 20px;align-items:center;gap:10px;min-height:62px;padding:9px 12px;border:1px solid transparent;border-radius:10px;background:transparent;color:var(--theme-text);text-align:left;text-decoration:none}.search-results a:hover,.search-results a:focus-visible{border-color:var(--theme-border);background:var(--theme-surface-muted)}.search-results a>span:nth-child(2){display:grid;gap:3px}@media(max-width:600px){.search-results a{grid-template-columns:70px 1fr 18px}}
</style>
