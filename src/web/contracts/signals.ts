...
export interface SignalsResponse extends IntelligenceMeta {
  ...
  consecutive: Array<MemberResult<SignalRowMeta & { direction: SignalDirection; consecutiveTradingDays: number; cumulativeActiveNetLots: number | null; participatingEtfs: number; sameDirectionEtfRatio: number | null; neutralEtfs: number; startDate: string | null; latestDate: string | null; actualObservationCount: number; missingObservationCount: number }>>;
  reversals: Array<MemberResult<SignalRowMeta & { reversalType: string; from: SignalDirection; to: SignalDirection; priorTradingDays: number; reversalDate: string | null; beforeActiveNetLots: number | null; afterActiveNetLots: number | null; majorityEtfDirectionFlip: boolean; participatingEtfs: number; sameDirectionEtfRatio: number | null }>>;
  divergences: Array<MemberResult<SignalRowMeta & { etfDirection: SignalDirection; institutionNetShares: number | null; relation: string; changedFromAligned: boolean; date: string | null }>>;
}
