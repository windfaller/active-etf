...
import { Building2, Calendar, Globe2, Home, Layers, ListChecks, RefreshCw, Search, X } from "@lucide/vue";
...
<template>
  <main class="app-shell p0-shell">
    ...
    <nav class="mobile-primary-nav" aria-label="行動版主要導覽">
      ...
      <button type="button" :class="{ active: route.view === 'globalMarket' || route.view === 'globalEtf' }" @click="navigate('/global-etfs')"><Globe2 :size="20" /><span>海外</span></button>
...
@media(max-width:760px){.p0-shell{gap:12px;padding:10px 10px calc(92px + env(safe-area-inset-bottom))}.p0-topbar{position:relative;grid-template-columns:1fr auto;min-height:58px;padding:8px 10px}.desktop-primary-nav,.top-actions .date-control{display:none}.top-actions{justify-self:end}.refresh-button{width:42px;height:42px}.area-subnav{justify-content:stretch;overflow:auto;padding-bottom:1px}.area-subnav button{flex:1 0 auto;min-height:44px}.p0-footer{display:grid;padding:16px 4px 8px}.p0-footer span{flex-wrap:wrap}.mobile-primary-nav{position:fixed;left:0;right:0;bottom:0;z-index:80;display:grid;grid-template-columns:repeat(4,1fr);padding:6px 8px calc(6px + env(safe-area-inset-bottom));border-top:1px solid #d7e0e4;background:rgba(255,255,255,.97);box-shadow:0 -8px 28px rgba(25,45,62,.09);backdrop-filter:blur(14px)}.mobile-primary-nav button{display:grid;justify-items:center;align-content:center;gap:3px;min-height:52px;border:0;border-radius:9px;background:transparent;color:#71808a;font-size:11px;font-weight:740}.mobile-primary-nav button.active{background:#eaf3f2;color:#0c756e}.mobile-search-overlay{position:fixed;inset:0;z-index:100;display:grid;align-items:end;background:rgba(7,22,34,.48)}.mobile-search-overlay>section{display:grid;gap:16px;padding:20px 16px calc(22px + env(safe-area-inset-bottom));border-radius:18px 18px 0 0;background:#fff;box-shadow:0 -20px 60px rgba(7,22,34,.18)}.mobile-search-overlay header{display:flex;justify-content:space-between;gap:16px}.mobile-search-overlay header>div{display:grid;gap:4px}.mobile-search-overlay header small{color:#71808a}.mobile-search-overlay header button{display:grid;place-items:center;width:44px;height:44px;border:1px solid #d9e1e5;border-radius:9px;background:#fff}.mobile-search-overlay label{display:grid;gap:6px;color:#5e6c77;font-size:12px;font-weight:760}.mobile-search-overlay select{width:100%;height:48px;padding:0 12px;border:1px solid #d4dee2;border-radius:9px;background:#fff}.institution-search-link{display:flex;align-items:center;gap:9px;min-height:48px;padding:0 13px;border:0;border-radius:9px;background:#173f56;color:#fff;font-weight:760}}
</style>
