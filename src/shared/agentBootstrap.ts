...
4. Use the client's native OAuth flow. Codex setup may start OAuth automatically; otherwise use \`codex mcp login active-etf\` or its MCP Authenticate control. Let the user complete member sign-in and read-only authorization in the browser. Never read another app's login tokens, request passwords/tokens in chat, or treat this installation link as authorization. The MCP client owns token storage and refresh. If the agent needs a restart/new conversation before tools become available, tell the user precisely and resume the check there.
