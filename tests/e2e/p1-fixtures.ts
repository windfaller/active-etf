...
const usOverview = {
  ...
  summary: { dataDate: "2026-07-20", lastUpdated: "2026-07-21T07:00:00.000Z", coveredEtfs: 2, primarySources: ["海外 ETF 發行商官方持股", "SEC 13F"] },
  today: null,
  overseasEtfExposure: { timeScale: "依發行商實際更新頻率", rows: [{ etfCode: "DRAM", fundName: "Roundhill Memory ETF", sourceAsOf: "2026-07-20", fetchedAt: "2026-07-21T07:00:00.000Z", sourceUrl: "https://example.com", sourceStatus: "ok", assetType: "Equity", weightPercent: 22.8, shares: 1000 }] },
  sec13f: { timeScale: "季度申報，非即時持倉", rows: [{ institutionCode: "ARK13F", institutionName: "ARK Investment Management", periodOfReport: "2026-03-31", filedAt: "2026-05-12", capturedAt: "2026-05-13T01:00:00.000Z", shares: 100, marketValue: 5000, weightPercent: 1.2, sourceUrl: "https://sec.gov" }] }
};

const history = { generatedAt: "2026-07-21T08:00:00.000Z", sourceAsOf: "2026-07-21", coverage: { tracked: 28, available: 24, delayed: 4 }, confidence: { level: "medium", reason: "部分延遲" }, market: "tw", symbol: "2330", window: 20, summary: { cumulativeActiveNetLots: 300, increaseTradingDays: 3, decreaseTradingDays: 1, consecutive: { direction: "increase", tradingDays: 3, startDate: "2026-07-17", latestDate: "2026-07-21", cumulativeActiveNetLots: 300 }, reversal: { detected: false, date: null, from: "unknown", to: "increase", priorTradingDays: 0 }, sameDirectionEtfRatio: .67, dataCoverageRate: .85 }, points: [{ date: "2026-07-21", activeNetLots: 120, direction: "increase", sameDirectionEtfRatio: .67 }, { date: "2026-07-18", activeNetLots: 100, direction: "increase", sameDirectionEtfRatio: .75 }, { date: "2026-07-17", activeNetLots: 80, direction: "increase", sameDirectionEtfRatio: .7 }] };
...
const comparison = { ...meta, type: "tw", cards: ["00981A", "00982A"].map((code) => ({ code, name: `${code} 測試 ETF`, issuer: "測試投信", sourceAsOf: "2026-07-21", fundSize: 1e9, premiumDiscount: .1, holdingCount: 50, top10Concentration: 55, hhi: .07, sectorExposure: [{ sector: "半導體", weight: 45 }], activeAdjustments: [{ window: 3, cumulativeActiveNetLots: 100, adjustmentIntensity: 2, increaseCount: 5, decreaseCount: 2 }], addedHoldings: 1, exitedHoldings: 0, dataCoverageRate: .9, topHoldings: [{ key: "2330|Equity", symbol: "2330", name: "台積電", assetType: "Equity", weight: 12 }] })), pairwise: [{ left: "00981A", right: "00982A", intersectionCount: 30, unionCount: 70, similarity: .428, weightedOverlap: 44, common: [{ key: "2330|Equity", label: "2330 台積電", leftWeight: 12, rightWeight: 10 }] }], methodology: { setOverlap: "intersection count 與 Jaccard similarity", weightedOverlap: "Σ min", commonDateOnly: true, missingWeight: "缺少權重不計", exposureIdentity: "代碼與類型" } };
const signals = { ...meta, market: "tw", window: 20, kind: "all", methodology: { tradingDays: "有效市場交易日", neutralThreshold: "極小變動為 neutral", consensus: "至少 2 檔且 60%", reversal: "前 2 日" }, consecutive: [{ stock: { market: "tw", symbol: "2330", name: "台積電", path: "/stocks/tw/2330" }, direction: "increase", consecutiveTradingDays: 3, cumulativeActiveNetLots: 300, participatingEtfs: 3, sameDirectionEtfRatio: .67, neutralEtfs: 1, startDate: "2026-07-17", latestDate: "2026-07-21", coverage: meta.coverage, confidence: meta.confidence }], reversals: [], divergences: [] };
...
export async function mockP1Apis(page: Page): Promise<void> {
  await page.route("**/api/**", async (route) => {
    ...
    if (url.includes("/stocks/") && url.includes("/etfs")) return route.fulfill({ json: { ...meta, rows: [{ etfCode: "00981A", name: "主動統一台股增長", latestWeight: 12, weightChange: .3, activeNetLots: 80, surfaceNetLots: 100, consecutiveDirection: "increase", consecutiveTradingDays: 3, dataDate: "2026-07-21", confidence: "high" }] } });
    if (url.includes("/stocks/") && url.includes("/institutions")) return route.fulfill({ json: { ...meta, timeScale: "台灣交易日", row: { foreignNetShares: 1000, investmentTrustNetShares: 200, dealerNetShares: -100, totalNetShares: 1100, source: "TWSE", relation: "aligned" }, rows: usOverview.sec13f.rows } });
    if (url.includes("/compare/etfs")) return route.fulfill({ json: comparison });
